...
@Component({
  ...
  templateUrl: './wordpress-plugins.page.html',
  styleUrls: ['./wordpress-plugins.page.scss'],
})
export class WordpressPluginsPage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options54 = {playerId: 'youtube-player54', playerSize: {width:270, height: 150}, videoId: 'Jo_rw8Td8Sk', debug: true};
    const result54 = await YoutubePlayerWeb.initialize(options54);
    console.log('playerReady', result54);

    (result54 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result54 as any).player.addEventListener('onStateChange', (event) => {
