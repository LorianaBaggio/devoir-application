...
@Component({
  ...
  templateUrl: './wordpress-blog.page.html',
  styleUrls: ['./wordpress-blog.page.scss'],
})
export class WordpressBlogPage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options49 = {playerId: 'youtube-player49', playerSize: {width:270, height: 150}, videoId: 'wbturmZn7pw', debug: true};
    const result49 = await YoutubePlayerWeb.initialize(options49);
    console.log('playerReady', result49);

    (result49 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result49 as any).player.addEventListener('onStateChange', (event) => {
