...
@Component({
  ...
  templateUrl: './html-head.page.html',
  styleUrls: ['./html-head.page.scss'],
})
export class HtmlHeadPage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options15 = {playerId: 'youtube-player15', playerSize: {width:270, height: 150}, videoId: 'Iu8YpBnhWm4', debug: true};
    const result15 = await YoutubePlayerWeb.initialize(options15);
    console.log('playerReady', result15);

    (result15 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result15 as any).player.addEventListener('onStateChange', (event) => {
