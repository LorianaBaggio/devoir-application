...
@Component({
  selector: 'app-sass-installation',
  templateUrl: './sass-installation.page.html',
  styleUrls: ['./sass-installation.page.scss'],
})
export class SassInstallationPage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options7 = {playerId: 'youtube-player7', playerSize: {width:270, height: 150}, videoId: 'MOstrhqpIsI', debug: true};
    const result7 = await YoutubePlayerWeb.initialize(options7);
    console.log('playerReady', result7);

    (result7 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result7 as any).player.addEventListener('onStateChange', (event) => {
