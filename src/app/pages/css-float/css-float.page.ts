...
@Component({
  ...
  templateUrl: './css-float.page.html',
  styleUrls: ['./css-float.page.scss'],
})
export class CssFloatPage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options25 = {playerId: 'youtube-player25', playerSize: {width:270, height: 150}, videoId: 'hfykeYvwZXs', debug: true};
    const result25 = await YoutubePlayerWeb.initialize(options25);
    console.log('playerReady', result25);

    (result25 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result25 as any).player.addEventListener('onStateChange', (event) => {
