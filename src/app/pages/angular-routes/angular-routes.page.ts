...
@Component({
  ...
  templateUrl: './angular-routes.page.html',
  styleUrls: ['./angular-routes.page.scss'],
})
export class AngularRoutesPage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options39 = {playerId: 'youtube-player39', playerSize: {width:270, height: 150}, videoId: 'kFl1wrSZ4sQ', debug: true};
    const result39 = await YoutubePlayerWeb.initialize(options39);
    console.log('playerReady', result39);

    (result39 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result39 as any).player.addEventListener('onStateChange', (event) => {
