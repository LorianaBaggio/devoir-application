...
@Component({
  ...
  templateUrl: './html-body.page.html',
  styleUrls: ['./html-body.page.scss'],
})
export class HtmlBodyPage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options17 = {playerId: 'youtube-player17', playerSize: {width:270, height: 150}, videoId: 'LkZ-KqxgVQ4', debug: true};
    const result17 = await YoutubePlayerWeb.initialize(options17);
    console.log('playerReady', result17);

    (result17 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result17 as any).player.addEventListener('onStateChange', (event) => {
