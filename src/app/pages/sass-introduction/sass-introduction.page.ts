...
@Component({
  selector: 'app-sass-introduction',
  templateUrl: './sass-introduction.page.html',
  styleUrls: ['./sass-introduction.page.scss'],
})
export class SassIntroductionPage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options6 = {playerId: 'youtube-player6', playerSize: {width:270, height: 150}, videoId: 'aOccUzHD_MQ', debug: true};
    const result6 = await YoutubePlayerWeb.initialize(options6);
    console.log('playerReady', result6);

    (result6 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result6 as any).player.addEventListener('onStateChange', (event) => {
