...
@Component({
  ...
  templateUrl: './angular-services.page.html',
  styleUrls: ['./angular-services.page.scss'],
})
export class AngularServicesPage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options40 = {playerId: 'youtube-player40', playerSize: {width:270, height: 150}, videoId: 'FRaVbFHRc7w', debug: true};
    const result40 = await YoutubePlayerWeb.initialize(options40);
    console.log('playerReady', result40);

    (result40 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result40 as any).player.addEventListener('onStateChange', (event) => {
