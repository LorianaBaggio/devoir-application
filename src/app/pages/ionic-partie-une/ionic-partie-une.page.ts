...
@Component({
  selector: 'app-ionic-partie-une',
  templateUrl: './ionic-partie-une.page.html',
  styleUrls: ['./ionic-partie-une.page.scss'],
})
export class IonicPartieUnePage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options8 = {playerId: 'youtube-player8', playerSize: {width:270, height: 150}, videoId: 'A0_qd9JOfd0', debug: true};
    const result8 = await YoutubePlayerWeb.initialize(options8);
    console.log('playerReady', result8);

    (result8 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result8 as any).player.addEventListener('onStateChange', (event) => {
