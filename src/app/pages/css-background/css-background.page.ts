...
@Component({
  ...
  templateUrl: './css-background.page.html',
  styleUrls: ['./css-background.page.scss'],
})
export class CssBackgroundPage implements OnInit, AfterViewInit {
  ...
  async initializeYoutubePlayerPluginWeb() {
    ...
    const options26 = {playerId: 'youtube-player26', playerSize: {width:270, height: 150}, videoId: 'OV0bOifr8AE', debug: true};
    const result26 = await YoutubePlayerWeb.initialize(options26);
    console.log('playerReady', result26);

    (result26 as any).player.addEventListener('onPlaybackQualityChange', (event) => {
      console.log('playback quality is', event);
    });

    (result26 as any).player.addEventListener('onStateChange', (event) => {
